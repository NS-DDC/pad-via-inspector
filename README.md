...
### 선택 인자 `dark_offset` — 어둡기 임계 조절

PAD 표면 얼룩을 VIA 로 잘못 잡으면 임계를 내려서 **더 어두운 것만** VIA 로 인정시킵니다.
...
```
   ...
   dark_offset ↓ (음수)   임계 낮아짐 → 아주 어두운 것만 VIA   → 얼룩 무시, 연한 VIA 도 놓침
   dark_offset ↑ (양수)   임계 높아짐 → 연한 것도 VIA          → 연한 VIA 검출, 얼룩도 딸려옴
```

PAD 밝기 중앙값에 **곱한 뒤 더하는** 순서라, 밝기가 다른 보드에서도 비율 부분은 그대로
따라가고 `dark_offset` 만 절대 보정으로 얹힙니다. 실제로 임계가 얼마가 됐는지는
`debug_via` 의 `rows[i]["dark_threshold"]` 로 확인하세요.

| `dark_offset` | PAD밝기 141 → 임계 |
|---|---|
| `0` (기본) | 87.4 |
| `-10` | 77.4 |
| `+20` | 107.4 |
| `-9999` / `+9999` | 1.0 / 254.0 (묶임, 예외 없이 동작) |

> **만능이 아닙니다.** 얼룩이 진짜 VIA 만큼 어두우면 같이 사라집니다. 실측에서도
> 대비 0.45 짜리 연한 VIA 와 얕은 얼룩은 어둡기가 겹쳐서, `dark_offset` 을 내리면
> 얼룩이 빠지는 대신 연한 VIA 가 `VIA_MISSING` 으로 빠졌습니다.
> `-3`, `-5` 씩 조금만 내리면서 `dark_threshold` 와 판정을 같이 보세요.
...
| `edge_rejected` | PAD 테두리에 붙어 **탈락**한 후보 수 (안 되살아남) |
| `offset_px`, `offset_norm` | 편심 절대/상대값 |
| `pad_median`, `dark_threshold` | 어둡기 임계 계산 근거 (`dark_offset` 반영된 값) |
...
flowchart TD
    ...
    C -->|"≥ 0.55"| D["③ VIA 후보 검출<br/>어둡기(+dark_offset) + 블랙햇"]
    D --> R{"④ 경계 여유<br/>≥ 반지름 × 0.30 ?"}
    R -->|아니오| X["탈락 (테두리 링)<br/>되살리지 않음"]
    R -->|예| S["⑤ 모양 우선순위<br/>원형인 것 먼저<br/>없으면 가장 큰 것"]
...
④ 만 **탈락**이고 ⑤ 는 **선호**입니다. ⑤ 에서 모든 후보가 모양에 걸려도 가장 큰 것을
...
`DARK_RATIO` 는 안 고쳐도 됩니다 — 같은 효과를 **호출할 때** `dark_offset` 으로 냅니다.
...
# "1" 양품 / "42" VIA 없음 / "99" VIA 쏠림 / "-1" 입력 오류
```
...
| 입력 | 원본 + 이진화 + 설계도 2장 | **VIA 이진화** + 설계도 2장 (+ 이진화) |
| 반환 | `(code, 결과이미지, via_bin)` | **`code`** |
| VIA 검출 | 어둡기 + 블랙햇 | **사용자가 준 마스크 그대로** |
| 판정 기준 | 편심·경계여유·면적·모양 | **동일** |
...
|---|---|
| 이미지 코드 일치 | **44 / 44** |
| PAD 판정 일치 | **783 / 783** (OK 705 · OFFSET 41 · MISSING 26 · ABSENT 5) |
...
[(4) 경계 여유](#4-pad-테두리를-via-로-오인하던-문제--경계-여유) 필터가 이 모듈에도 그대로 들어 있습니다.
...
```python
code = check_via_code(VIA이진화, PAD설계도, VIA설계도, bin_mask=이진화, verbose=True)
```

반환값은 그대로 코드 하나이고, 표만 추가로 찍힙니다.
...
VIA_MIN_CLEARANCE = 0.30  VIA_MAX_ASPECT     = 2.2    VIA_MAX_RADIAL_DEV = 0.45
PAD_PRESENT_MIN   = 0.55  VIA_EXCLUDE_RATIO  = 0.75
```

`DARK_RATIO`·블랙햇·`PAD_ERODE` 는 **없습니다.** VIA 를 찾는 단계가 없기 때문입니다.
