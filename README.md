...
설계도를 2px 만 밀면 정중앙 PAD 들이 통째로 쏠림으로 뒤집힙니다.
...
PAD 하나하나에 대해 설계 형상을 실측 마스크 무게중심 쪽으로 조금 평행이동합니다
(최대 `PAD반지름 × ALIGN_MAX_RATIO`). 전역 정합은 쓰지 않습니다 — 설계 PAD 가
실물보다 작게 그려져 있어 겹침이 ±2px 구간에서 평평해지고, 최대값이 잡음으로 결정됩니다.

이 보정은 **진짜 쏠림은 오히려 강화합니다.** 정중앙 VIA 구멍은 대칭이라 무게중심을
밀지 않지만, 쏠린 VIA 구멍은 무게중심을 반대쪽으로 밀어 편심이 커집니다.
44장 실측 기준 정중앙 최대 0.1311 → 0.1186, 쏠림 최소 0.4130 → 0.4478,
**분리 여유 0.282 → 0.329.**

**(2) VIA 때문에 PAD 영역에 구멍이 생겨 `PAD_ABSENT` 로 빠지던 문제 — 이중 커버리지**

이진화에서 VIA 자리가 통째로 뚫리면 커버리지가 무너집니다.
`full`(설계 PAD 전체 대비)과 `excl`(VIA 자리를 뺀 영역 대비)을 둘 다 재고 **큰 값**을 씁니다.

| 구멍 반경 | `full` 만 | `max(full, excl)` |
|---|---|---|
| 2 px | ABSENT 1 | ABSENT 1 |
| 3 px | ABSENT **11** | ABSENT 1 |
| 4 px | ABSENT **159** | ABSENT 2 |

`excl` 은 구멍에 면역이고 `full` 은 실물 PAD 가 설계보다 작을 때(침식) 강합니다.
큰 값을 쓰면 둘의 장점만 남습니다.
