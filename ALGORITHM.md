...
> **단독 모듈과의 차이** — `via_checker.py` 에는 이 엔진에 없는 후보 선별 두 가지가
> 더 있습니다. PAD 표면 노이즈가 심한 실물 이미지를 위한 것입니다.
>
> | | 내용 |
> |---|---|
> | 모양 우선순위 | 후보 중 **원형인 것**(장단비 ≤ 2.2, 반경편차 ≤ 0.45)을 먼저 고릅니다. 원형 후보가 하나도 없으면 모양을 포기하고 가장 큰 것을 쓰므로, 이 필터가 `VIA_MISSING` 을 새로 만들지는 못합니다 |
> | `dark_offset` | 호출 인자. 임계 = `PAD밝기중앙값 × DARK_RATIO + dark_offset` (1~254 로 묶임) |
>
> 두 기능 모두 깨끗한 이미지에서는 판정을 바꾸지 않아 44장 테스트셋 결과가
> 엔진과 동일합니다 (PAD 783/783). 실측 근거는 README 의
> "실물 이미지에서 나던 오판정 세 가지" 를 보세요.
