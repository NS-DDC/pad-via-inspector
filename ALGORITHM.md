...
> **단독 모듈과의 차이** — `via_checker.py` 는 검출 1차 조건 자체가 다르고,
> 이 엔진에 없는 후보 선별이 더 붙어 있습니다. PAD 표면 노이즈가 심한 실물
> 이미지를 위한 것입니다.
>
> | | 성격 | 내용 |
> |---|---|---|
> | 색상차 검출 | **1차 조건** | `‖화소BGR − PAD대표색BGR‖₁ > 색상차중앙값 + 3.5 × 로버스트편차` 를 씁니다. 엔진의 `밝기 < 중앙값 × 0.62` 를 대체합니다 |
> | Black-hat / Top-hat | **2차 조건** | 어두운 VIA 와 밝은 이물을 모두 잡도록 두 응답 중 큰 쪽을 씁니다 |
> | 모양 우선순위 | **선호** | 후보 중 **원형인 것**(장단비 ≤ 2.2, 반경편차 ≤ 0.45)을 먼저 고릅니다. 원형 후보가 하나도 없으면 모양을 포기하고 가장 큰 것을 쓰므로, 이 필터가 `VIA_MISSING` 을 새로 만들지는 못합니다 |
> | 경계 여유 | **탈락** | 덩어리의 최대 경계거리가 `PAD반지름 × 0.30` 미만이면 버립니다. 되살리지 않습니다 |
> | 채움비 | **탈락** | `면적 / 최소외접원면적 < 0.45` 면 버립니다. 되살리지 않습니다 |
> | `dark_offset` | 호출 인자 | 색상차 임계 = `색상차중앙값 + 3.5 × 로버스트편차 − dark_offset` |
>
> **왜 색상차로 바꿨나.** 밝기 비율은 PAD 색이 라인마다 다르거나 VIA 대비가 연하면
> 임계가 어긋납니다. 색상차는 PAD 마다 자기 대표색을 기준으로 재는 상대값이라
> 같은 숫자를 그대로 씁니다. "PAD 안에서 PAD 색과 다르면 VIA" 라는 정의를 그대로
> 수식으로 옮긴 것입니다.
>
> **경계 여유·채움비만 성격이 다릅니다.** PAD 테두리를 두른 어두운 링은 장단비 1.0 /
> 반경편차 0.13~0.20 이라 모양으로는 완벽한 원이고, 무게중심도 PAD 중심과 겹쳐
> 편심이 0 이 됩니다. 즉 **VIA 가 아예 없는 PAD 가 양품으로 둔갑**합니다(불량 놓침).
> 모양으로는 못 잡고 위치·전체 모양으로만 잡히며, 되살리면 안 되는 이유이기도 합니다.
> 실측 분리도 : 경계여유 진짜 VIA 최소 0.536 vs 테두리 링 중앙 0.254,
> 채움비 꽉 찬 원 0.6~0.9 vs 링·호 0.2 아래.
>
> 이 기능들 모두 깨끗한 이미지에서는 판정을 바꾸지 않아 44장 테스트셋 결과가
> 엔진과 동일합니다 (PAD 772/783, 테두리탈락 0). 실물 49장에서는 오검출이
> 146 → **7** 로 줄었습니다. 실측 근거는 README 의
> "실물 이미지에서 나던 오판정 네 가지" 를 보세요.
>
> **`via_code.py`** 는 여기서 검출 단계만 떼어낸 판정 전용 모듈입니다. 사용자가
> 만든 VIA 이진화를 받아 `(코드, 결과이미지)` 를 돌려줍니다. 판정 기준은
> `via_checker.py` 와 동일해서 `via_bin` 을 그대로 넣으면 같은 코드가 나옵니다
> (44/44, PAD 777/777).
