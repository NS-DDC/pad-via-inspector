...
> **단독 모듈과의 차이** — `via_checker.py` 에는 이 엔진에 없는 후보 선별 세 가지가
> 더 있습니다. PAD 표면 노이즈가 심한 실물 이미지를 위한 것입니다.
>
> | | 성격 | 내용 |
> |---|---|---|
...
> | `dark_offset` | 호출 인자 | 임계 = `PAD밝기중앙값 × DARK_RATIO + dark_offset` (1~254 로 묶임) |
>
> **경계 여유만 성격이 다릅니다.** PAD 테두리를 두른 어두운 링은 장단비 1.0 /
> 반경편차 0.13~0.20 이라 모양으로는 완벽한 원이고, 무게중심도 PAD 중심과 겹쳐
> 편심이 0 이 됩니다. 즉 **VIA 가 아예 없는 PAD 가 양품으로 둔갑**합니다(불량 놓침).
> 모양으로는 못 잡고 위치로만 잡히며, 되살리면 안 되는 이유이기도 합니다.
> 실측 분리도 : 진짜 VIA 최소 0.536 vs 테두리 링 중앙 0.254.
>
> 세 기능 모두 깨끗한 이미지에서는 판정을 바꾸지 않아 44장 테스트셋 결과가
> 엔진과 동일합니다 (PAD 783/783, 테두리탈락 0). 실측 근거는 README 의
> "실물 이미지에서 나던 오판정 네 가지" 를 보세요.
>
> **`via_code.py`** 는 여기서 검출 단계만 떼어낸 판정 전용 모듈입니다. 사용자가
> 만든 VIA 이진화를 받아 코드 문자열만 돌려줍니다. 판정 기준은 `via_checker.py` 와
> 동일해서 `via_bin` 을 그대로 넣으면 같은 코드가 나옵니다 (44/44, PAD 783/783).
